...
## Create `Event` field for labeling - in this example the frequency column is being renamed. 
...
```{r}
#Need to add a column with the Event year before combining. 
#If importing multiple water surface profiles files, set the file path where all the profile data is being stored.
path <- system.file("extdata/longitudinal_profiles", 
                    package = "razviz")
...
## Set Event as an ordered factor
profile_data_ordered$Event <- factor(profile_data_ordered$Event,
                              levels = model_events,
                              labels = model_events)

hydro_model_1 <- profile_data_ordered
```
