...
#in the example .csv files the  column with the return periods was labelled 'Freq'. 
...
#Filter Longitudinal Profile to Shorter Reach

##Plot Saint Anthony Falls to LD 1
Due to the large elevation drop at Saint Anthony Falls, we will create two sets of plots, split at RM 843.3 (halfway between LD1 tailwater and St. Paul, MN)
```{r LSFtoLD1 Filter, eval=FALSE}
LSFtoLD1 <- dplyr::filter(profile_data_ordered, River_Sta >= 843.3)
high_water_events_df_LSFtoLD1  <- dplyr::filter(high_water_events_df, river_mile >= 843.3)
gages_LSFtoLD1  <- dplyr::filter(gages, river_mile >= 843.3)
gage_labels_df_LSFtoLD1  <- dplyr::filter(gage_labels_df, river_mile >= 843.3)
gage_boxes_df_LSFtoLD1  <- dplyr::filter(gage_boxes_df, river_mile >= 843.3)
bridges_LSFtoLD1 <-  dplyr::filter(bridges, river_mile >= 843.3)
features_LSFtoLD1 <-  dplyr::filter(features, river_mile >= 843.3)
levees_LSFtoLD1 <-  dplyr::filter(levees, river_mile >= 843.3)
```

```{r LSFtoLD1 Plot, eval=FALSE}
filename <- "Longitudinal_Profile_Report_Coon Rapids_LD1.pdf"

#Plot Labels
plot_labels <- list("title" = "Upper Mississippi River Hydraulic Model - Coon Rapids, MN to Lock and Dam No. 10",
                    "x_axis" = "Miles Above the Ohio River",
                    "y_axis" = "Elevation (NAVD88 feet)")

#longitudinal plot pages
max(LSFtoLD1$River_Sta)-min(LSFtoLD1$River_Sta)
miles_per_plot <- 22.84

long_plot_pages_df_LSFtoLD1 <- razviz::long_plot_pages(LSFtoLD1, high_water_events_df_LSFtoLD1, miles_per_plot)

report(hydro_model = LSFtoLD1,
       long_plot_pgs = long_plot_pages_df_LSFtoLD1,
        gages = gages_LSFtoLD1,
        gage_labels_df = gage_labels_df_LSFtoLD1,
        gage_boxes_df = gage_boxes_df_LSFtoLD1,
        high_water = high_water_events_df_LSFtoLD1,
        levees = levees_LSFtoLD1,
        features = features_LSFtoLD1,
        bridges = bridges_LSFtoLD1,
        graph_colors = graph_cols,
        legend_labels = legend_labels,
        plot_labels = plot_labels,
        output_dir = output_dir)
