...
#Option #1 - automatic import from dss file
...
#Manually Add 2D Flows - if drew a profile line across the 2D area and copy and pasted it out of the HEC-RAS GUI
```{r}
#read in csv files
...
for (y in CX_2D){
 ...
 for(p in 1:length(plan_year)){ #start with a specific plan
    Plan_Event <- plan_year[p]
...
##Import Observed Data from csv files (Option #2)
* Use the `razviz::import_csv_manualrasoutput` function to import the example RAS model results included in the package for each modeled event. * Specify the hydrologic event name, the model run number, and the model type. 
